...
{
    internal const int Failure =
        unchecked((int)0x80004005);

    private readonly ConcurrentQueue<(
        byte[]? Packet,
        int Result,
        Action? Action)> _dispatch = new();
    ...
    internal ConcurrentQueue<(
        uint DefinitionId,
        string DatumName,
        string Units)> DataDefinitions { get; } = new();
    ...
    internal ConcurrentQueue<(
        ...
        uint ObjectId)> TelemetryRequests { get; } = new();

    internal int TelemetryRequestResult { get; set; }

    internal ConcurrentQueue<(
        uint EventId,
        string EventName)> SystemEvents { get; } = new();
    ...
    internal ConcurrentQueue<(
        ...
        bool TouchAndGo)> EnrouteCreates { get; } = new();

    internal int CreateEnrouteResult { get; set; }

    internal ConcurrentQueue<(
        uint ObjectId,
        uint RequestId)> RemoveRequests { get; } = new();

    internal int RemoveResult { get; set; }
        ...
        nint notificationEvent)
    {
        ...
        try
        {
            ...
            int result = OpenResults.TryDequeue(
                out int queued)
                ? queued
                : 0;
            ...
            {
                OpenedBeforeClose |=
                    Interlocked.Increment(ref _liveHandles) != 1;
                handle = attempt;
            }
        ...
        string unitsName)
    {
        ...
        try
        {
            DataDefinitions.Enqueue(
                (definitionId, datumName, unitsName));
        ...
        SimConnectPeriod period)
    {
        ...
        try
        {
            TelemetryRequests.Enqueue(
                (requestId, definitionId, period, 0));
        ...
        SimConnectPeriod period)
    {
        ...
        {
            TelemetryRequests.Enqueue(
                (requestId, definitionId, period, objectId));
            return TelemetryRequestResult;
        }
        finally
        {
    ...
    public int SubscribeToSystemEvent(
        nint handle,
        uint eventId,
        string eventName)
    {
    ...
    private void Enter()
    {
        ...
        OverlapDetected |=
            Interlocked.Increment(ref _activeCalls) != 1;
    }
...
internal static class SimConnectPackets
{
            ...
            "Microsoft Flight Simulator 2024")
    {
        ...
        {
            BitConverter.GetBytes(version[i])
                .CopyTo(bytes, 268 + i * 4);
        }
    ...
    internal static byte[] Exception(uint code)
    {
        ...
        BitConverter.GetBytes(code)
            .CopyTo(bytes, 12);
        BitConverter.GetBytes(19u)
            .CopyTo(bytes, 16);
        BitConverter.GetBytes(4u)
            .CopyTo(bytes, 20);
        return bytes;
    }
    ...
    internal static byte[] AssignedObjectId(
        uint requestId,
        uint objectId)
    {
        byte[] bytes = Header(12, 20);
        BitConverter.GetBytes(requestId)
            .CopyTo(bytes, 12);
        BitConverter.GetBytes(objectId)
            .CopyTo(bytes, 16);
        return bytes;
    }

    internal static byte[] SimObjectData(
        uint requestId,
        uint definitionId,
    ...
    {
        byte[] bytes =
            Header(8, 40 + values.Count * sizeof(double));
        BitConverter.GetBytes(requestId)
            .CopyTo(bytes, 12);
        BitConverter.GetBytes(objectId)
            .CopyTo(bytes, 16);
        BitConverter.GetBytes(definitionId)
            .CopyTo(bytes, 20);
        BitConverter.GetBytes((uint)values.Count)
            .CopyTo(bytes, 36);

        for (int i = 0; i < values.Count; i++)
        {
            BitConverter.GetBytes(values[i])
                .CopyTo(bytes, 40 + i * sizeof(double));
        }

        return bytes;
    }

    internal static byte[] SystemState(
        uint requestId,
        uint inFlight = 0)
    {
        ...
        BitConverter.GetBytes(requestId)
            .CopyTo(bytes, 12);
        BitConverter.GetBytes(inFlight)
            .CopyTo(bytes, 16);
        return bytes;
    }

    internal static void WithPointer(
        byte[] bytes,
        Action<nint, uint> action)
    {
        nint data =
