...
**Parallel military branch:** `feature/military-conflict-system`, draft PR #10 -> `feature/m1-simulation-core`; deterministic ground/air conflict, all current military mission families, authorization, fictional theater generation, SQLite campaign/mission recovery, strategic phase/momentum/objectives, persisted threat outcomes, dispatch authorization and the Military/Government snapshot projection are implemented there. Current verified military implementation head `018e1da5` is green: Linux passed 402/402 xUnit + 29/29 SimLab. The latest production WinUI head `555b9dc7` built WinUI/live-probe with 0 errors and passed 402/402 xUnit on Windows; the exact-head `018e1da5` Windows workflow also passed with its build job intentionally skipped because that final slice changed tests only.
...
## Built
**Parallel conflict foundation (PR #10):** deterministic ground/air conflict, multi-family missions, abstract effects/threat damage, qualification/assignment authorization, fictional theater generation, SQLite campaign recovery, strategic campaign evolution, persistent player-operation state and a UI-ready operations projection are implemented on `feature/military-conflict-system`. Automatic startup recovery, SQLite-consistent backup/restore, deterministic operation/faction identity and postures, bounded campaign cycles, finite replacements, terminal outcomes, history, successor planning/actions, and the first production Military/Government WinUI screen are implemented. The screen projects active operation/factions/postures/outcome/reserves/front/support/objectives and successor Accept/Decline/Reconsider through application services only. Shared SQLite is reconciled at schema v3 so prior FlightSession-v2 and conflict-v2 databases both gain the missing parallel table. Completed-operation history, an interactive read-only completed-operation drill-down with stable polling selection, persisted terminal facts, keyboard/focus UX, SQLite restart fidelity and no-write viewing verification, persisted faction-posture evolution, a read-only schematic Operational Map, and deterministic current-state Communications are implemented. Remaining: user-facing restore picker/confirmation, main-career/fleet/job/economy integration, local visual acceptance and live gameplay verification.
