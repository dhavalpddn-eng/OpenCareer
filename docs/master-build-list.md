...
Isolated foundation on `feature/mbl17-airframe-condition`: permanent physical AirframeId plus separate wear/discrete damage persistence (schema 14), without changing the frozen KJFK playable loop. Slice 2 persists trustworthy first-contact landing observations. Slice 3 persists canonical model plus optional explicitly validated physical airframe on career FlightSessions, retaining model-only operation and legacy checkpoint compatibility. Slice 4 adds schema-15 exact-once terminal consequences for explicitly assigned physical airframes, using retained contact evidence and verified historical gameplay calibration; condition mutation/history commit atomically before checkpoint clear. Slice 5 gates explicit physical assignments before acceptance/reservation and rechecks before start using only RequiresGrounding; model-only operation remains unchanged. Slice 6 exposes bounded read-only consequence history and current serviceability from the existing schema-15 authority. Slice 7 adds exact-once discrete-damage repair with unchanged wear, atomic schema-16 immutable service history and separate bounded service queries. Slice 8 adds schema-17 exact-tick usage, explicit migration baseline, a versioned historical 50-hour gameplay inspection schedule, exact-once condition-preserving inspection events and a separate inspection-due dispatch gate. Slice 9 adds read-only fallback reliability with explicit unavailable component/probability data and a failure-eligibility boundary that cannot generate failures. Slice 10 adds an optional unwired engine-1 SimConnect actuator with fresh dedicated readback and conservative toggle/retry safety on the existing worker. Slice 11 adds schema-18 exact physical-airframe landing-cycle tracking and independent provenance from authoritative `FlightAirframeSummary.LandingEpisodeCount` inside the existing exact-once consequence transaction. Migrated rows use a zero migration baseline without historical backfill; confirmed cycles survive a later non-crash interruption; one landing episode remains one cycle regardless of bounce contacts; new inspection history retains cycle-complete v3 evidence while legacy v2 remains readable. Condition/calibration, dispatch, economy and failure actuation remain unchanged. Maintenance remains incomplete: verified aircraft-specific/component schedules, wear servicing, costs, downtime, component reliability, authorized automatic failure orchestration and Maintenance UI are deferred; native actuator effects await live validation.
