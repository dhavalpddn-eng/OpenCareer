...
The user clarified the 50–80 real-flight-hour target: a small aircraft bought outright or a larger aircraft financed, depending on career standing and affordability. There is no forced ownership hour gate. The balance center remains a fictional $1,000 net savings per career-credit hour. Including the current $250 initial storage and $420 standard-hull first-period cost, the neutral $60,000 cash path and $250,000 / 20%-deposit financed path each require about **64.67 hours** before dealer promotions. The fresh-job payout envelope is now 0.85–1.15x the center, and worst-case current dealer promotions are tested together with that envelope so an optimized legitimate path still does not cross below 50 hours while an ordinary low-paying fresh path stays below 80. These are game tuning values, not live wages/prices or guaranteed loans. End-to-end play balance still requires the playable settlement loop and real operating-cost integration.
...
Time acceleration is allowed but must not multiply qualifying career hours **or player profit**. Career hours already use real-equivalent credit. Economy settlement now scales only the player-profit component by career-credit time / simulated movement time, capped at 1.0; direct operating-cost reimbursement is not reduced. Thus 4x simulation cannot create 4x real-world earnings, while 0.5x does not penalize the player. Pause/slew does not accrue progress; material slew/position teleport cannot advance normal career jobs; changing aircraft during an active job is not allowed. Full design: [flight session design](flight-session-design.md).
...
The named-commodity catalog and deterministic commodity-market/freight quote core now exist. Declared cargo value can increase security/insurance exposure, but its freight surcharge is capped at 25% of transport pay so a tiny high-value electronics load cannot become the obvious money farm. Binding accepted manifests to FlightSession/jobs and authoritative cargo settlement remains downstream.

## Management depth

Maintenance, financing, dispatch preparation and ground handling default to automation. Eligible manual ground procedures can provide a small bounded benefit. The reward policy caps direct reward at both $35 per flight and $35 per career-credit hour, deduplicates procedure kinds within one quote, rounds to cents and requires a completed, high-confidence user-confirmed procedure. A 10-minute qualifying flight therefore cannot turn the fixed flight cap into a farming loop. This remains a quote until authoritative flight-scoped evidence and one transactional settlement per flight exist. Automated handling must not reduce baseline pay.
