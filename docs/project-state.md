...
- The military branch is synchronized with current `feature/m1-simulation-core` FlightSession/OpenAI work. Shared SQLite migration is now schema v3 and repairs either legacy parallel-v2 shape by idempotently ensuring both `flight_session_checkpoint` and `conflict_campaigns` exist.
- Code head `4b0472b4` is green: Linux 352/352 xUnit + 29/29 SimLab; Windows WinUI/live-probe builds at 0 errors + 352/352 xUnit.
- Code head `b95e8f2e` is green: Linux 260/260 xUnit + 29/29 SimLab; Windows WinUI/live-probe builds at 0 errors + 260/260 xUnit.
- Still open: user-facing Settings restore selection/confirmation, optional dynamic posture changes across later campaign phases/outcomes, richer Military/Government map/comms/history presentation, local visual/accessibility acceptance, authoritative career/dispatch/job settlement integration, large balance/stress runs and live telemetry gameplay verification.
