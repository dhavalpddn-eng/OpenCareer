...
3. MBL-06 is complete and parked at the publication boundary: production `FlightSessionRuntime` publishes accepted authoritative `FlightStateEvidence` through read-only `IFlightStateEvidenceSource`, with deterministic fresh-sample, disconnect, reconnect, duplicate-sample and reset behavior. Consumer-specific checklist/tutorial/jobs integration belongs to those owning lanes; consumers must not recompute simulator evidence or mutate FlightSession state.
