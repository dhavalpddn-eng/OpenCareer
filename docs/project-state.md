...
2. Add a separately verified source/bridge for remote-airport weather if destination preflight weather is required; do not use deprecated SimConnect weather-station APIs or reuse user-position ambient wind remotely.
3. Model richer documented `flight_performance.cfg` takeoff/landing/cruise tables while preserving their weight/OAT/altitude dimensions; do not flatten them into unsafe single-number limits.
