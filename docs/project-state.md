...
- Parallel military/conflict implementation: `feature/military-conflict-system`, draft PR #10 targeting `feature/m1-simulation-core`. Ground/air conflict, all current military mission families, authorization, fictional theater generation, deterministic operation/faction identity and operational posture, SQLite recovery/startup resume, strategic evolution, logistics recovery/control cycles, finite replacement reserves, terminal Victory/Defeat/Stalemate/Ceasefire outcomes, persistent completed-operation history, deterministic successor planning/transition, application-layer successor offer decisions, SQLite-consistent backup and validated staged restore are implemented. Code head `e41161bb` is green on Linux/Windows. See `docs/conflict-system.md`.
...
- Still open: user-facing Settings restore selection/confirmation, Military/Government successor-offer presentation, optional dynamic posture changes across later campaign phases/outcomes, authoritative career/dispatch/job settlement integration, production Military/Government UI, large balance/stress runs and live telemetry gameplay verification.
