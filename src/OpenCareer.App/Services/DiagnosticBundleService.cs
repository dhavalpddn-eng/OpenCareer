    ...
    ILogger<DiagnosticBundleService> logger)
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        string fileName =
            $"opencareer-diagnostics-{DateTimeOffset.Now:yyyyMMdd-HHmmss-fff}.zip";
        ...
        try
        {
            ...
            await using (FileStream output = File.Create(temporary))
            {
                           ...
                           leaveOpen: true))
                {
                    ZipArchiveEntry reportEntry =
                        archive.CreateEntry("diagnostic-report.json");
                    ...
                    await using (Stream reportStream = reportEntry.Open())
                    {
                        await JsonSerializer.SerializeAsync(
                            ...
                            new
                            {
                                ...
                                application = new
                                {
                                    version = typeof(DiagnosticBundleService)
                                ...
                                simulator = new
                                {
                                    ...
                                    applicationVersion =
                                        connectionSnapshot.Simulator?.ApplicationVersion.ToString(),
                                    simConnectVersion =
                                        connectionSnapshot.Simulator?.SimConnectVersion.ToString()
                                },
            ...
            logger.LogInformation(
                "Created diagnostic bundle at {Path}.",
                destination);
            ...
            ex is IOException or UnauthorizedAccessException or InvalidDataException)
        {
            ...
            return new SettingsActionResult(
                false,
                $"Diagnostic export failed: {ex.Message}");
        }
    }
