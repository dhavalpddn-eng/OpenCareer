...
public sealed partial class SqliteAirframeStore : IAirframeServiceStateStore
{
        ...
        SqliteTransaction? transaction, AirframeId id, CancellationToken cancellationToken)
    {
        ...
        command.CommandText = """
            SELECT airframe_id, schedule_id, schedule_version, total_airborne_ticks, total_landing_cycles,
                landing_cycle_origin, last_inspection_airborne_ticks, next_inspection_airborne_ticks,
                usage_origin, revision, updated_at_utc_ticks
        ...
        if (reader.GetValue(0) is not string || reader.GetValue(1) is not string
            || Enumerable.Range(2, 9).Any(i => reader.GetValue(i) is not long))
        ...
        var result = new AirframeServiceState(id, reader.GetString(1), checked((int)reader.GetInt64(2)),
            TimeSpan.FromTicks(reader.GetInt64(3)), TimeSpan.FromTicks(reader.GetInt64(6)), TimeSpan.FromTicks(reader.GetInt64(7)),
            (AirframeUsageOrigin)checked((int)reader.GetInt64(8)), reader.GetInt64(9), new(reader.GetInt64(10), TimeSpan.Zero),
            reader.GetInt64(4), (AirframeUsageOrigin)checked((int)reader.GetInt64(5)));
        result.Validate();
        return result;
    }
        ...
        AirframeServiceState state, long? expectedRevision, CancellationToken cancellationToken)
    {
        ...
        command.CommandText = expectedRevision is null ? """
            INSERT INTO airframe_service_state (airframe_id, schedule_id, schedule_version, total_airborne_ticks,
                total_landing_cycles, landing_cycle_origin, last_inspection_airborne_ticks,
                next_inspection_airborne_ticks, usage_origin, revision, updated_at_utc_ticks)
            VALUES ($id, $schedule, $version, $total, $cycles, $cycleOrigin, $last, $next, $origin, $revision, $updated);
            """ : """
            UPDATE airframe_service_state SET total_airborne_ticks=$total, total_landing_cycles=$cycles, last_inspection_airborne_ticks=$last,
            ...
            WHERE airframe_id=$id AND schedule_id=$schedule AND schedule_version=$version
                AND usage_origin=$origin AND landing_cycle_origin=$cycleOrigin AND revision=$expected;
            """;
        ...
        command.Parameters.AddWithValue("$cycles", state.TotalTrackedLandingCycles);
        command.Parameters.AddWithValue("$cycleOrigin", (int)state.LandingCycleOrigin);
