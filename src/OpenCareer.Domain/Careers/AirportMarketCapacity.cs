...
{
    public static AirportMarketCapacity ForScale(
        AirportMarketScale scale) =>
        scale switch
        {
            AirportMarketScale.Local =>
                new(scale, 10, 0.10, 0.10, 0.65),
            AirportMarketScale.Small =>
                new(scale, 16, 0.25, 0.20, 0.70),
            AirportMarketScale.Regional =>
                new(scale, 20, 0.45, 0.35, 0.65),
            AirportMarketScale.Large =>
                new(scale, 35, 0.70, 0.65, 0.45),
            AirportMarketScale.MajorHub =>
                new(scale, 45, 0.90, 0.80, 0.30),
            AirportMarketScale.MegaHub =>
                new(scale, 60, 1.00, 0.95, 0.25),
    ...
    public void Validate()
    {
        ...
        if (MatureVisibleOfferCapacity is < 1 or > 100)
            throw new ArgumentOutOfRangeException(
                nameof(MatureVisibleOfferCapacity));

        ValidateUnit(
            PassengerActivityIndex,
            nameof(PassengerActivityIndex));
        ValidateUnit(
            CargoActivityIndex,
            nameof(CargoActivityIndex));
        ValidateUnit(
            GeneralAviationActivityIndex,
            nameof(GeneralAviationActivityIndex));
    }
