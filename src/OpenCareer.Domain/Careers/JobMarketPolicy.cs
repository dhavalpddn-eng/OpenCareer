...
public static class JobMarketAccessExtensions
{
    public static bool Allows(
        this JobMarketAccess access,
        ServiceTrack track) =>
        track switch
        {
            ServiceTrack.CivilianEmployment =>
                access.HasFlag(JobMarketAccess.CivilianEmployment),
            ServiceTrack.IndependentContract =>
                access.HasFlag(JobMarketAccess.IndependentContract),
            ServiceTrack.CompanyContract =>
                access.HasFlag(JobMarketAccess.CompanyContract),
            ServiceTrack.GovernmentContract =>
                access.HasFlag(JobMarketAccess.GovernmentContract),
            ServiceTrack.MilitaryService =>
                access.HasFlag(JobMarketAccess.MilitaryService),
            _ => false
        };
}
...
{
    public static JobMarketPolicy Default { get; } =
        new(
            MinimumOffers: 1,
            MaximumOffers: 2,
    ...
    public TimeSpan EffectiveMinimumOfferLifetime =>
        MinimumOfferLifetime ?? TimeSpan.FromHours(2);

    public TimeSpan EffectiveMaximumOfferLifetime =>
        MaximumOfferLifetime ?? TimeSpan.FromHours(18);

    public void Validate()
    {
        if (MinimumOffers < 1
            || MaximumOffers < MinimumOffers
            || MaximumOffers > 10)
        {
        ...
        RequirePositiveFinite(
            DistanceDecayNm,
            nameof(DistanceDecayNm));
        RequirePositiveFinite(
            MaximumGeneratedDistanceNm,
            nameof(MaximumGeneratedDistanceNm));
        RequireUnit(
            LongRangeFloorWeight,
            nameof(LongRangeFloorWeight));
        RequireNonNegativeFinite(
            EstablishedRouteBoost,
            nameof(EstablishedRouteBoost));
        RequireNonNegativeFinite(
            RelationshipBoost,
            nameof(RelationshipBoost));
        RequirePositiveFinite(
            GovernmentTrackBoost,
            nameof(GovernmentTrackBoost));
        RequirePositiveFinite(
            MilitaryTrackBoost,
            nameof(MilitaryTrackBoost));
        RequirePositiveFinite(
            UasSpecialtyBoost,
            nameof(UasSpecialtyBoost));
        RequireUnit(
            CivilianEmploymentShare,
            nameof(CivilianEmploymentShare));
        RequireUnit(
            IndependentContractShare,
            nameof(IndependentContractShare));
        RequireUnit(
            CompanyContractShare,
            nameof(CompanyContractShare));
        RequireUnit(
            LockedPreviewWeight,
            nameof(LockedPreviewWeight));
        RequirePositiveFinite(
            LocalOperationWeight,
            nameof(LocalOperationWeight));

        if (MaxEquivalentOffersPerCycle is < 1 or > 10)
        {
            throw new ArgumentOutOfRangeException(
                nameof(MaxEquivalentOffersPerCycle));
        }

        if (!double.IsFinite(BoardGrowthExponent)
            || BoardGrowthExponent <= 0)
        {
            throw new ArgumentOutOfRangeException(
                nameof(BoardGrowthExponent));
        }
        ...
        if (FirstDreamPreviewBoardSize < 1
            || SecondDreamPreviewBoardSize < FirstDreamPreviewBoardSize)
        {
            throw new ArgumentOutOfRangeException(
                nameof(FirstDreamPreviewBoardSize));
        }

        if (EffectiveMinimumOfferLifetime <= TimeSpan.Zero
            || EffectiveMaximumOfferLifetime < EffectiveMinimumOfferLifetime)
        {
            throw new ArgumentOutOfRangeException(
                nameof(MinimumOfferLifetime));
        }

        if (EarlyCareerLevelCeiling < 1
            || EarlyCareerLevelCeiling > CareerLevelCap)
        {
            throw new ArgumentOutOfRangeException(
                nameof(EarlyCareerLevelCeiling));
        }

        RequirePositiveFinite(
            EarlyCareerPreferredMinHours,
            nameof(EarlyCareerPreferredMinHours));

        if (!double.IsFinite(EarlyCareerPreferredMaxHours)
            || EarlyCareerPreferredMaxHours < EarlyCareerPreferredMinHours)
        {
            throw new ArgumentOutOfRangeException(
                nameof(EarlyCareerPreferredMaxHours));
        }

        RequireUnit(
            EarlyCareerOutOfBandWeight,
            nameof(EarlyCareerOutOfBandWeight));

        double civilianShare =
            CivilianEmploymentShare
            + IndependentContractShare
            + CompanyContractShare;
        ...
        if (Math.Abs(civilianShare - 1.0) > 1e-9)
        {
            throw new ArgumentException(
                "Civilian service-track shares must sum to one.");
        }
    }

    public int VisibleOfferCount(
        ...
        DeterministicRandom random)
    {
        ...
        double grown =
            startingCount
            + (capacity.MatureVisibleOfferCapacity - startingCount)
            * Math.Pow(progress, BoardGrowthExponent);
    ...
    public int DreamPreviewCount(
        int boardCount,
        bool lockedTracksExist)
    {
        Validate();

        if (!ShowLockedPreviews
            || !lockedTracksExist
            || MaximumDreamPreviews == 0)
        {
            return 0;
        }
        ...
        DeterministicRandom random)
    {
        ...
        if (scenario == JobScenarioKind.OrganTransport)
        {
            return TimeSpan.FromMinutes(
                random.NextDouble(15, 90));
        }

        TimeSpan min =
            EffectiveMinimumOfferLifetime;
        TimeSpan max =
            EffectiveMaximumOfferLifetime;

        if (kind is ContractKind.ExpressCargo
            or ContractKind.AogPartsDelivery
            or ContractKind.Medical
            or ContractKind.Medevac)
        {
    ...
    public double DurationSuitability(
        ContractKind kind,
        ...
        int careerLevel)
    {
        ...
        if (hours >= EarlyCareerPreferredMinHours
            && hours <= EarlyCareerPreferredMaxHours)
        {
            return 1.35;
        }

        double distanceFromBand =
            hours < EarlyCareerPreferredMinHours
        ...
        double weight =
            Math.Max(
                EarlyCareerOutOfBandWeight,
                Math.Exp(-distanceFromBand / 1.5));

        if (kind is ContractKind.Ferry
        ...
        DateTimeOffset time)
    {
        ...
        long ticks =
            checked(index * RefreshInterval.Ticks);
